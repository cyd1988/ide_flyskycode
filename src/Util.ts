...
export class Util  {
    ...
    static getJsonData(document: vscode.TextDocument|any=null, content:string|null=null) {
        if (!document) {
            document = vscode.window.activeTextEditor ? vscode.window.activeTextEditor.document : null;
        }
        if (!content) {
    ...
    static getSystemInfo(func:Function) {
        ...
        exec(bash, data, function(error: string | null, stdout: any, stderr: any) {
            if (error !== null) {
                ...
                show_text = show_text.replace("Num fans: 2 Fan 0 - Left side   at ", ', ');
                show_text = show_text.replace("Fan 1 - Right side  at ", ', ');
                func(show_text);

                // view.erase_status('prefixr_cpu_fan')
                // view.set_status('prefixr_cpu_fan', stdout)
                // sublime.set_timeout(lambda: view.erase_status('prefixr_cpu_fan'), 9000)
     ...
     */
    static getExtensionFileAbsolutePath(context: { extensionPath: any; }, relativePath: any) {
     ...
     * @param relativePath 扩展中某个文件相对于根目录的路径，如 images/test.jpg
     */
    static getExtensionFileVscodeResource(context: { extensionPath: any; }, relativePath: any) {
