...
class Highlight {
    ...
    private ranges: any
    static obj: Highlight;


    public static init() {
        if (!this.obj) this.obj = new Highlight();
        return this.obj;
    }
    ...
    public setMode(m: any) { this.mode = m }
    public getMode() { return this.mode }
    public getWords() { return this.words }
    public setDecorators(d: any) { this.decorators = d }
        ...
        window.visibleTextEditors.forEach(editor => {
            ...
            this.words.forEach((w, naa) => {
                ...
                while (match = regEx.exec(text)) {
                    ...
                    let aa = decs.length < 1 ? 0 : naa % decs.length;
                    if (!decs[aa]) decs[aa] = [];
                    decs[aa].push(decoration);
